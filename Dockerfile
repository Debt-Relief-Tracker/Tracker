...
#     E.g.: docker.io/hexpm/elixir:1.19.0-erlang-28.0.2-debian-trixie-20260610-slim
...
# Self-hosted SQLite default (docs/architecture/0001-dual-database-adapter.md):
# operators bind-mount a host directory here to persist the database file.
# Set DATABASE_URL instead to use Postgres -- DATABASE_PATH is then ignored.
RUN mkdir -p /data && chown nobody:root /data
VOLUME ["/data"]
ENV DATABASE_PATH="/data/debt_tracker.db"

# set runner ENV
ENV MIX_ENV="prod"

# Only copy the final release from the build stage
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/debt_relief_tracker ./
